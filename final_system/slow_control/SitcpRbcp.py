...
This is a bug of the SitcpRbcp module (not a bug of user program)'
        ...
        #print reply_ver_type;
        #print reply_cmd_flag;
        #print reply_id;
        #print reply_length;

        if reply_ver_type != 0xff:
            raise ValueError, 'reply packet Ver/Type is not 0xff (but %02x)' % (reply_ver_type)
        #ackbit_mask = 0x80 # original # bus access ???
        ackbit_mask = 0x08 # modify@20170801
        if (reply_cmd_flag & ackbit_mask) != ackbit_mask:
            raise ValueError, 'reply packet does not have Ack bit'
        busserr_mask = 0x01 # added@20170801
        if (reply_cmd_flag & busserr_mask) != 0x00:
            raise ValueError, 'reply packet have Buss Error bit'
