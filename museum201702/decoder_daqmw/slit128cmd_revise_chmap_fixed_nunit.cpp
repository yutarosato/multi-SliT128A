...
int fill_event_buf( FILE *fp, unsigned char *event_buf, int n_unit ){ // 0(correctly read one event), -1(end event (or fread err))
  int n;
...
int main( int argc, char *argv[] ){
  ...
  if( argc<8 ){
    std::cerr << "Usage : "
	      << argv[0] << "  "
	      << "output_rootfile  input_binary_file  VREF0 VREF1 VREF23 HV n_unit" << std::endl;
    abort();    
  }else{
    out_filename = argv[1];
