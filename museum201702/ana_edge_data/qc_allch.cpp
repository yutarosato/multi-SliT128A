...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  for( Int_t ievt=0; ievt<chain->GetEntries(); ievt++ ){
    ...
    for( Int_t ivec=0; ivec<t_unit_v->size(); ivec++ ){
      for( Int_t ibin=0; ibin<t_tedge_v->at(ivec) - t_ledge_v->at(ivec); ibin++ ){
	hist    ->Fill( t_ledge_v->at(ivec)+ibin, multi_ch_map(t_chip_v->at(ivec),t_unit_v->at(ivec),t_bit_v->at(ivec)) );
	hist_int->Fill( t_ledge_v->at(ivec)+ibin, multi_ch_map(t_chip_v->at(ivec),t_unit_v->at(ivec),t_bit_v->at(ivec)) );
      }
    }
